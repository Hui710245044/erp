<template>
    <page class="table-list" style="">
        <mailing-method ref="mailing">
            ...
            <el-table slot="goods-tabel"
                ...
                <el-table-column
                        ...
                        label="最后同步时间"
                        inline-template>
                    <ui-tips style="cursor: pointer" @click.native="viewLog(row)"
        ...
        <page-dialog :title="`同步${channelTitle}状态历史`" v-model="viewVisible" size="large" :close-on-click-modal="false">
            <p>订单号：{{rowNumber}}</p>
            <el-table :data="viewLogRow">
                ...
                <el-table-column
                        ...
                        label="同步结果"
                        inline-template>
                    <span>{{row.synchronize_result}}</span>
                </el-table-column>
            </el-table>
        </page-dialog>
