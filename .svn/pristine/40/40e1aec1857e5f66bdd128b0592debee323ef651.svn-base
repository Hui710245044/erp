<template>
    <page class="over-flow-auto" v-loading="loading" element-loading-text="数据加载中...">
        ...
        <div class="page-fixed">
            ...
            <request-button reqKey="syncUpc" :mintime="300"
                            v-if="(param.curPage==='not'||param.curPage==='copy'||param.curPage==='draft')&&form.data.length>1"
                            @click="sync_upc">同步UPC & PartNumber
            </request-button>
            <request-button :req-key="`translate${currentname}`"
                            :disabled="comDisabled"
                            @click="translate_cur">仅翻译（{{currentname}}）账号</request-button>
            <request-button reqKey="translateUseAll"
                            :disabled="comAllDisabled"
                            @click="translate_all">翻译所有账号</request-button>
            <el-button type="primary" size="mini" @click.native="keep_draft">保存到草稿箱</el-button>
            <request-button :reqKey="`${comReqKey}`"
                            @click="public_channel">刊登到平台
            </request-button>
            <el-button size="mini" @click="do_cancel">取消</el-button>
        </div>
    </page>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            * */
            ...
            get_old_data(data){
                ...
                return this.$http(api_publish_amazon_edit,params).then(res => {
                    ...
                    this.$message({type:'error',message:'当前站点暂无可引用的数据'});
                })
            },
            /*引用历史数据接口
            * */
            quote_old(index){
                ...
                if(length<=0){
                    this.loading = false;
                    return this.$message({type:"warning",message:"暂无可引用的历史数据"});
                }
           ...
           * */
            quote_model_data(index,res){
                ...
                this.get_edit_data(type,obj.siteId,res.category_template_id, res.product_template_id).then(cateObj=>{
                    ...
                    if(!existCate&&!existPro){
                        this.reset_product_category_attr(this.param, cateObj, res,'分类模板和产品模板');
                    }else if(!existCate&&existPro){
                        this.reset_product_category_attr(this.param, cateObj, res,'分类模板');
                    }else if(existCate&&!existPro){
                        this.reset_product_category_attr(this.param, cateObj, res,'产品模板');
                    }else{
                        /*存在产品模板和分类模板时
                       *整理产品模板&&分类模板所带的属性
                       * */
            ...
            translate_all() {
                this.$confirm('将翻译所有已选账号，请确保每个账号的数据已填写完整！ 点击【确定】按钮后执行翻译，注意处理避免等待过程中的重复点击。', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    let findList = this.form.data.filter(row=>!row.is_translate);
                    if(findList.length<=0)return this.$message({type:'warning',message:'所需翻译的数据为空：US,UK,CA,AU 站点数据将不会被翻译，已翻译过的数据也不会被翻译'});
            ...
            translate_api(list, name) {
                this.$http(api_translate, list).then(res => {
                    ...
                    this.$message({type: 'success', message: '翻译完成'});
                }).catch(code => {
                    this.$message({type: 'error', message: code.message || code})
                }).finally(() => {
                    if (name === 'cur') {
            ...
            async get_all_rule_attr(siteId, spu, accountId) {//后端给我的字段和规则
                if (this.allRuleAttr.hasOwnProperty(`${accountId}-${siteId}`)) return;
                await this.$http(api_amazon_field, {site: siteId, spu: spu}).then(res => {
            ...
            async get_attribute(row) {
                ...
                await this.get_product_attribute(productType, row.site).then(id => {
                    ...
                    * */
                    ...
                    if (!id) return this.$message({type: "warning", message: `【${row.code}】账号，暂无默认的产品模板，请添加！`});
            ...
            * */
                ...
                this.$http(api_publish_amazon_edit, {id: params.id, copy: copy}).then(res => {
                    ...
                    * */
                    this.get_edit_data(type, res.site, res.category_template_id, res.product_template_id).then(item => {
                        ...
                        if(!existCate&&!existPro){
                            this.reset_product_category_attr(params, item, res,'分类模板和产品模板');
                        }else if(!existCate&&existPro){
                            this.reset_product_category_attr(params, item, res,'分类模板');
                        }else if(existCate&&!existPro){
                            this.reset_product_category_attr(params, item, res,'产品模板');
                        }else{
                            /*存在产品模板和分类模板时
                           *整理产品模板&&分类模板所带的属性
                           * */
            ...
            handle_change(val) {
                val.forEach(row => {
                    ...
                    this.get_all_rule_attr(row.site, this.param.spu, row.account_id);//获取产品&分类属性
                });
            },
            assign_info(val) {
            ...
            copy_data(val) {
                val.forEach(row => {
                    ...
                    let notTranslateSite = ['US','UK','CA','AU'];/*不翻译的站点集合*/
                ...
                if (!!length) {
                    this.currentTab = val[length - 1].code;
                    this.currentname = val[length - 1].code;
                }
            },
            //添加时数据渲染事件
            add_data(val) {
                let notTranslateSite = ['US','UK','CA','AU'];/*不翻译的站点集合*/
                val.forEach(row => {
                    let descript = this.allRuleAttr[`${row.account_id}-${row.site}`].descript;/*描述*/
                    let basic = this.allRuleAttr[`${row.account_id}-${row.site}`].basic;/*基本信息*/
                    let images = this.allRuleAttr[`${row.account_id}-${row.site}`].img.map(row => row.data);/*图片*/
                    ...
                    let variant_option = this.allRuleAttr[`${row.account_id}-${row.site}`].variant_option;/*sku设置绑定变体*/
                    let params = {
                        siteId: row.site,
                        code: row.code,
                        doc_id:this.allRuleAttr[`${row.account_id}-${row.site}`]['header']['doc_id'],
                        currency: row.currency,
                        account_id: row.account_id,
            ...
            validate_data() {
                ...
                this.form.data.forEach(row => {
                    ...
                    if (!row.siteId) return canSubmit = this.public_message(row.code, '站点');
                    if (!row.category_template_id) return canSubmit = this.public_message(row.code, `【基本信息】模块->分类模板`);
                    if (!row.product_template_id) return canSubmit = this.public_message(row.code, `【基本信息】模块->产品模板`);
                    let findRecommend= row.basic.find(row=>row.key==='RecommendedBrowseNode'&&row.value==='');
                    if(findRecommend) return canSubmit = this.public_message(row.code, `【基本信息】模块->RecommendedBrowseNode属性`);
                    let findBasic = row.basic.find(res => res.require && res.value === '');
                    if (findBasic) return canSubmit = this.public_message(row.code, `【基本信息】模块->${findBasic.name}`);
                    let findCommonAttr = row.commonAttList.find(res => res.require && res.value === '');
                    if (findCommonAttr) return canSubmit = this.public_message(row.code, `【基本信息】模块->${findCommonAttr.name}`);
                    /*判断标题与描述是否必填*/
                    ...
                    if (listDescript.length > 0) {
                        ...
                        return canSubmit = this.public_message(row.code, `【标题与描述】模块->SKU:${findDescript.skuName}->${this.which_title(findDescript.find.key)}`);
                    }
                    /*判断标题与描述部分是否超出最大字符长度*/
                    ...
                    if (listDes.length > 0) {
                        ...
                        return canSubmit = this.limit_maxLength(row.code, `【标题与描述】模块->SKU:${findDes.skuName}->${this.which_title(findDes.find.key)}`);
                    }
                    /*判断SKU设置是否必填*/
                    ...
                    if (listVariant.length > 0) {
                        ...
                        return canSubmit = this.public_message(row.code,`【SKU设置】模块->第${findVariant.index + 1}行->${findVariant.find.name}`);
                    }
                    let findImage = row.images.find(res => res.name !== '主图' && res.images.length <= 0);
                    if (findImage) {
                        this.$message({
                            type: "warning",
                            message: `${row.code}账号，【图片与描述】模块->子SKU:${findImage.name}至少添加一张图片`
                        });
                        canSubmit = false;
                    }
                });
...
//                            if(row.key==="Title"||row.key==="BulletPoint"||row.key==="Description" ){
            ...
            public_message(code, name, otherTip){
                if (otherTip) {
                    this.$message({type: 'warning', message: `【${code}】账号，${name}有必填项未填写，请补充完整`});
                } else {
                    this.$message({type: 'warning', message: `【${code}】账号，${name}为必填项，请补充完整`});
                }
                return false;
            },
            //五点描述提示
            five_points_tip(row){
                ...
                let message = '';
                if(obj.isSame){
                    this.$message({type:'warning',message:'Bullet-Point 存在重复项'});
                    return false
                }
                switch (obj.key){
                    case 'Title':
                        message = `【标题与描述】模块，sku：【${obj.curSku}】->【标题】重复率不能高于90%`;
                        break;
                    case 'BulletPoint':
                        message = `【标题与描述】模块,sku:【${obj.curSku}】->【Bullet-Point】->第【${obj.curIndex+1}】重复率不能高于95%`;
                        break;
                    case 'Description':
                        message = `【标题与描述】模块,sku:【${obj.curSku}】->【详情描述】重复率不能高于95%`;
                        break;
                }
                this.$message({type:'warning',message:`【${row.code}】账号，${message},当前重复率为${Number(obj.curPercent).toFixed(2)*100}%`});
                return obj.isTrue;
            },
            //字符长度限制
            limit_maxLength(code, name) {
                this.$message({type: 'warning', message: `【${code}】账号，${name}字符长度超出最大限制，请修改`});
                return false;
            },
            ...
            keep_draft() {
                ...
                this.$http(api_publish_amazon_draft, params).then(res => {
                    ...
                    return this.$message({type: 'success', message: res.message || res})
                }).catch(code => {
                    return this.$message({type: 'error', message: code.message || code});
                })
            },
            // 刊登到平台
            public_channel() {
        ...
        computed: {
            referableNum(){
                ...
                return this.historyIdObj[find.siteId].length
            },
            comDisabled(){
                let cur = false;
