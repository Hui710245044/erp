<template>
    <div class="p-look-list">
        <page-dialog v-model="addVisable" @open="open" size="large" :title="`查看采购单号：${lookData.purchase_order_code} 信息`" @change="change_dialog" width="80%" left="10%">
            <el-row>
                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="采购仓库：" prop="warehouse_id" required>
                                ...
                                <el-select v-if="edit" v-model="lookData.warehouse_id" placeholder="请选择" class="default-width">
                                    <el-option
                                        :key="item.id"
                        ...
                        <el-col :span="12">
                            <el-form-item label="采购人员：" prop="purchase_user_id" required>
                                ...
                                <el-select v-if="edit" v-model="lookData.purchase_user_id" placeholder="请选择" class="default-width">
                                    <el-option
                                        :key="item.id"
                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="供应商：" prop="supplier_id" required>
                                ...
                                <el-select v-if="edit" v-model="lookData.supplier_id" placeholder="请选择" class="default-width">
                                    <el-option
                                        :key="item.id"
                        ...
                        <el-col :span="12">
                            <el-form-item label="实际应付货款：">
                                ...
                                <el-input v-if="edit&&!isShow" v-model="lookData.payable_amount"  class="default-width"></el-input>
                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="币种：">
                                <div><span>{{currency}}</span>&nbsp;&nbsp;&nbsp;<span v-if="currency!=='CNY'">({{`1 ${currency}=${lookData.rate} CNY`}})</span></div>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="实际已付款：">
                                <span v-if="!edit">{{lookData.actual_payment | filterNumber}}</span>
                                <el-input v-if="edit" v-model="lookData.actual_payment" class="default-width"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="货款：">
                                ...
                                <el-input v-if="edit&&!isShow" v-model="lookData.amount" class="other-width"></el-input>
                        ...
                        <el-col :span="12">
                            <el-form-item label="优惠金额：">
                                ...
                                <el-input v-if="edit&&!isShow" v-model="lookData.discount_amount" class="other-width"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row>
            ...
            <card  label="物流信息" class="mt-sm">
                <el-form :model="lookData" label-width="90px" label="left" ref="form2" :rules="rules">
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="外部流水号：" style="padding-left: 30px;">
                                ...
                                <el-input v-if="isEdit" v-model="lookData.external_number" @change="check_chinese(lookData)" class="default-width inline"></el-input>
                                <el-button type="primary" size="mini" class="ml-md inline" @click="get_tracking_number" :disabled="editAble || stepsStatus !== 2" v-if="lookData.external_number">获取运单号</el-button>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="运费：" class="inline" style="padding-left: 30px;">
                                <el-input v-if="lookData.payment_status == '5'&& isEdit" v-model="lookData.shipping_cost" style="width:150px" class="inline" type="number" min="0" step="0.01" @blur="blur_shipping_cost()"></el-input>
                    ...
                    <el-row v-for="(item,index) in lookData.logistics" :key="item.id">
                        <el-form :model="item" ref="item" :rules="rules">
                            <el-col :span="4">
                                ...
                                <el-form-item label="运输方式：" class="inline" style="width: 190px;padding-left: 5px;" prop="logistics_company_name" required>
                                    <span v-if="isEdit">
                                          <allow-create-select v-model="item.logistics_company_name" style="width: 120px;"></allow-create-select>
                                    </span>
                                    <span v-else>{{item.logistics_company_name}}</span>
                                </el-form-item>
                            </el-col>
                            <el-col :span="4">
                                <el-form-item label="运单号：" style="margin-left: 25px" prop="logistics_bill_no" required>
                                    ...
                                    <span class="select-link-btn" v-else>
                                        <span class="link" @click="get_logistics(lookData.purchase_order_code,item)">{{item.logistics_bill_no}}</span>
                                    </span>
                                </el-form-item>
                            </el-col>
                            <el-col :span="4">
                            ...
                            <el-col :span="4">
                                ...
                                <div class="mt-mini inline" @click="delete_tracks(index)" v-if="index>0&&isEdit&&item.is_from_input == 1">
                    ...
                    <el-row v-for="(item,index) in logisticsData" :key="index" v-if="isEdit && addEdit" v-model="addEdit">
                        <el-form :model="item" ref="item" :rules="rules">
                            <el-col :span="4">
                    ...
                    <div class="fr" v-if="!isEdit">
                        <el-button size="mini" type="primary" v-if="lookData.status !== 21 || lookData.status !== 20 || lookData.status !== -1" @click="edit_logistics" :disabled="lookData.status == 21||lookData.status == -1 || editAble">编辑</el-button>
                    </div>
                    <div class="fr" v-else>
            ...
            <card  label="商品信息"  class="mt-sm">
                ...
                <div  class="fr mb-xs mr-xs" v-if="isShowCheck">
                    ...
                    <el-checkbox class="inline" :indeterminate="isIndeterminate" v-model="checkAllStatus" @change="handleCheckAllChange">全选</el-checkbox>
                    <el-checkbox-group v-model="checkedStatus" class="inline ml-sm" @change="handleCheckedStatusChange">
                        <el-checkbox v-for="status in statusLists" :label="status" :key="status">{{status}}</el-checkbox>
                    </el-checkbox-group>
                </div>
                <el-table
                    ...
                    <el-table-column
                        ...
                        min-width="50"
                        label="是否为替换品">
                        <select-text :edit="edit" v-model="row.is_replace" placeholder="请选择" @input="chang_(row)">
                            <el-option
                                :key="item.value"
                    ...
                    <el-table-column
                        ...
                        label="备注">
                        <!--isEdit_remark-->
                        <div title="点击可更改备注" @click="cur_click(row)" v-if="(lookData.status==0 || lookData.status==10)&&!row.isEdit_remark">
            ...
            <card label="采购计划备注" class="mt-sm p-look-plan-remark">
                <div class="p-look-plan-remark-list">
                    备注：<div class="p-look-plan-remark-list" v-html="filterRemark"></div>
                </div>
            </card>
            <card label="操作日志" class="mt-sm">
                <el-table
                    :data="actionLog"
                    height="200"
                    highlight-current-row
            ...
            <div class="mt-xs" v-if="!editAble&&!isInvalidApply&&!isInvalidAudit">
                ...
                <el-row><el-button type="primary" size="mini" class="fr mt-xs" @click="add_mark" :disabled="isMark">添加备注</el-button></el-row>
            </div>
            <div class="row mt-sm" v-if="editAble||isInvalidApply||isInvalidAudit">
                ...
                <image-upload v-if="isInvalidApply" :thumbnail-mode="true" label="上传附件" class="inline mt-sm mb-lg"  ref="images" :init="file"></image-upload>
            </div>
...
<script>
    ...
    import {
        ...
        api_purchase_order_remarks} from "../../../api/purchase";
    ...
    export default{
        ...
        methods:{
            ...
            add_track(){
                ...
                if(delivery_time!==0){
                    let expect_arrive_time = new Date(create_time).getTime() + 3600 * 1000 * 24 * delivery_time;
                    this.logisticsData.push({logistics_bill_no:'',
                        logistics_company_name:'',
                        delivered_time:'',
            ...
            keep(){
                ...
                pram.sku_json={add,mod,del:this.delSkuid
                }
