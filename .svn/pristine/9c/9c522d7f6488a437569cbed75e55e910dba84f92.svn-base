...
//                    datef日期处理函数，把后面的参数转化为前面的样式
            ...
            package_delete() {
                this.$confirm(`所勾选包裹将被处理为：包裹作废，确认此操作吗？`, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
        ...
        computed: {//实例计算属性
            ...
            isBatchHandle() {
                return this.searchData.exception_type === 2 || this.searchData.exception_type === 8 || this.searchData.exception_type === 7;
            },
            batchButtons() {
                let buttons = this.abnormalList;
                if (buttons.length === 1) {
                    return buttons;
                } else {
