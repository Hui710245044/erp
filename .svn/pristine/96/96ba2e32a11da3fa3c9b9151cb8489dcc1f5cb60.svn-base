    ...
    <div class="purchase-plan-edit-table">
        <ui-table
                ...
                <tr>
                    ...
                    <td>
                        <div>
                            ...
                            <span v-else-if="tableData.supplier&&row.hasSupplier" title="点击更换供应商" class="operate"
            ...
            <template>
                ...
                <tr>
                    <td colspan="18" style="text-align: left;padding: 5px 10px">
...
<script>
    ...
    export default {
        ...
        methods: {
            currencyChange(val) {
                this.$set(this.tableData, 'currency_code', val);
                this.tableData.goods.forEach(row => {
                    this.$set(row, 'currency_code', val);
                });
            },
            get_currency_list() {
            ...
            chang_(row) {
                let index = this.tableData.goods.indexOfFun(item => {
                    return item.sku_id === row.sku_id
                });
                if (!this.tableData.goods[index].action) {
                    this.tableData.goods[index].action = "mod"
                }
            },
            chang_Supplier() {
                if (!this.tableData.action) {
                    this.tableData.action = "mod"
                }
            },
            analyze(data) {
        ...
        computed: {
            ...
            headTitles() {
                return [
                    ...
                    {label: '最小起订数', size: 40},
                    {label: '采购数量', size: 55},
                    {label: `采购单价${this.currency_code ? `(${this.currency_code})` : ''}`, size: 60},
                    {label: `最后一次采购单价${this.currency_code ? `(${this.currency_code}` : ''})`, size: 60},
                    {label: '3月内最低报价', width: 6},
                    {label: `区间报价${this.currency_code ? `(${this.currency_code})` : ''}`, width: 6},
                    {label: '结算方式', size: 50},
                    {label: '采购员', size: 45},
                    {label: '操作', size: 60}
                ]
