    ...
    <page class="plan-index">
        <search-card @search="search_list" @enter="search_list" :params="params" :clears="clears">
            ...
            <div class="el-row mb-mini">
                <el-select v-sf.snType v-model="params.snType"
                ...
                <order-input v-if="notIsSPU" v-model="params.snText"
                             class="inline s-width-large"
                             v-sf.snText
                             @keydown="search_list"
                             placeholder="可批量搜索，Shift+回车换行..."></order-input>
                <el-input v-else v-model="params.snText"
                          v-sf.snText
                ...
                <el-select v-model="params.shType"
                ...
                <!--审核人的搜索-->
                <span v-if="params.shType === 'approve_user_id'">
                   <purchaser class="inline s-width-small"
                              v-sf.approve_user_id
                              v-model="params.approve_user_id">
                   </purchaser>
                </span>
                <!--创建人的搜索-->
                <span v-else="params.shType === 'create_user_id'">
                    <purchaser class="inline" v-sf.create_user_id v-model="params.create_user_id"></purchaser>
                </span>
                <label class="inline ml-sm" title="请选择采购员">采购员：</label>
                <purchaser class="inline s-width-small" v-sf.purchase_user_id
                           v-model="params.purchase_user_id"></purchaser>
            ...
            <label class="inline" title="请选择提交日期"> 提交日期：</label>
        ...
        <el-table class="mb-sm scroll-bar"
            ...
            <el-table-column label="审核人" min-width="50" inline-template>
                <div class="p-table-list-td-text" :title="row.approver_name">{{row.approver_name}}</div>
            </el-table-column>
...
<script>
    ...
    export default {
                ...
                pickerstart: {
                    disabledDate: (time) => {
                        if (this.params.date_e) {
                            return time.getTime() > this.params.date_e.getTime()
                        } else {
                            return false
                        }
                    }
                ...
                pickerend: {
                    disabledDate: (time) => {
                        if (this.params.date_b) {
                            return time.getTime() < this.params.date_b.getTime()
                        } else {
                            return false
                        }
                    }
        ...
        methods: {
            ...
            init_params() {
                let params = {
                    ...
                    purchase_user_id: this.params.purchase_user_id,
                    create_user_id: this.params.create_user_id,
                    approve_user_id: this.params.approve_user_id,
                };
                ...
                if (this.params.date_b) {
                    params.date_b = datef('YYYY-MM-dd', this.params.date_b / 1000);
                } else {
                    params.date_b = '';
                }
                if (this.params.date_e) {
                    params.date_e = datef('YYYY-MM-dd', this.params.date_e / 1000);
                } else {
                    params.date_e = "";
                }
                if (this.notIsSPU) {
            ...
            approval() {
                this.id = this.selects;
                let data = {
                    id: this.selects.join(',')
                };
                            ...
                            this.$http(api_plan_look, data).then(res => {
                                this.lists.forEach(row => {
                                    if (row.id === data) {
                                        ...
                                        this.$set(row, 'warehouse_name', res.warehouse);
                                        this.$set(row, 'kind', total);
                                        typeof res.amount === 'number' && res.amount.toString();
                                        this.$set(row, 'amount', Number(res.amount).toFixed(3));
                                        this.$set(row, 'supplier_list', supplier.join(','));
                                        this.$set(row, 'purchaser_list', purchaser.join(','));
                                    }
                                });
                            }).catch(code => {
                                this.$message({message: code.message || code, type: 'error'});
            ...
            look_init(data, row) {
                if (row.purchase_plan_type === 'purchase_order_overcharged') {
                    this.selectMark = 'other';
                } else if (row.purchase_plan_type === 'ready_purchasing') {
                    this.selectMark = 'stocking';
                } else {
                    this.selectMark = 'normal';
                }
                this.loading = true;
            ...
            operate_click_dispose(row) {
                this.id = row.id;
                this.operates[row.behavior].event.call(this, row);
            },
            operate_switch_dispose(row, index) {
                row.behavior = index;
                this.operate_click_dispose(row);
        ...
        computed: {
            ...
            notIsSPU() {
                return this.params.snType.toUpperCase() !== 'SPU'
            },
            emptyText() {
