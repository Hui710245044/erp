<template>
    <page class="print-p-index">
        <search-card @search="search" @enter='search' :params="searchData" :clears="clears">
            ...
            <label >模板类型：</label>
            <el-select v-sf.temp_type v-model="searchData.temp_type" placeholder="请选择" class="mr-sm inline s-width-default">
                <el-option
    ...
    <el-row class="ml-sm mb-xs mt-xs">
        <permission
                ...
                class="inline" size="mini" type="primary" @click.native="add">添加</permission>
        <!--&lt;!&ndash;<el-button  class="inline" size="mini" type="primary" @click.native="add">添加</el-button>&ndash;&gt;-->
        <permission
                tag="requestButton"
                :route="apis.url_label_del"
                class="inline" :request="del" :disabled="selected.length <= 0">删除</permission>
...
<script>
    import {api_label_list,api_label_type,api_label_edit,api_label_del,api_label_edit_copy,url_label_save,url_label_del,url_label_edit_copy,url_label_edit} from "../../../api/print"
    export default{
        permission:{
