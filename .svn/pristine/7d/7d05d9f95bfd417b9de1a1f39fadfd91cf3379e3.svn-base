                        ...
                        <template v-if="item.select === 0">
                            <el-form-item v-if="item.key==='Title'"
                                ...
                                <el-button style="margin-left: 30px;" type="primary" size="mini"
                                           @click="initial_uppercase(row,i)">首字母大写
                                </el-button>
                            </el-form-item>
                            ...
                            <el-row v-else-if='item.key==="Description"'>
                                <el-col :span="4"  style="text-align:right">
                                    <label>详情描述 <b>（<span :class="[item.value.length>item.maxLength?'red':'color-success']">{{item.value.length}}</span>/{{item.maxLength}}）</b>：</label>
                                </el-col>
                                <el-col :span="18">
                                    ...
                                    <div v-if="exceedMaxLength" class="mt-sm" style="font-size:1.25rem">
                                        ...
                                        <el-button type="danger" size="mini" v-if="index===0" :mintime="300" :disabled="isDisabled" @click.native="apply_sku">应用到子SKU</el-button>
                                    </div>
                                </el-col>
                            </el-row>
                            <!--<el-form-item v-else-if='item.key==="Description"'-->
                                          <!--:label="`详情描述：（${item.value.length}/${item.maxLength}）`">-->
                                <!--<div style="width:90%">-->
                                    <!--<simditor v-model="item.value"-->
                                              <!--ref="uEditor"-->
                                              <!--@value-changed="content_change(index,i,$event)"-->
...
//                        res.field[i].value = val;
            ...
            initial_uppercase(row,index){
                if(!row.field[index].value) return this.$message({message:"请输入刊登标题", type:'warning'});
