...
<script>
    ...
    // import print_websocket from '@/mixins/print_websocket';


    export default {
        // mixins: [print_websocket],
        data() {
            return {
                searchData: {
                    shipping_number: '',
        ...
        methods: {
            ...
            weighing(shipping_number) {//需要称重
                shipping_number && (this.searchData.shipping_number = shipping_number);
                this.$refs.search_module.$refs.package_weight.select();
                this.pollingWeight();
            },
            async createPackage(response) {
                ...
                if (channel.shipping_id !== response.main_shipping_id) {
            ...
            pollingWeight() {
                /*let data = {
                    ...
                    id: `${this.searchData.shipping_number}-${Date.now()}`,
                    sendmode: 0,
                    package_id: this.searchData.shipping_number,
                    callback: ''
                };
                this.$sendRequest(data, (res) => {
                    if(res.status === 1) {
                        console.log(`xxxxxx:socket return`);
                        this.$refs.search_module.searchData.package_weight = res.direct_results;
                        this.searchData.package_weight = res.direct_results;
                        this.submitWeight();
                    }
                });*/
            ...
            start_whole(direction) {
                if (this.isWholeEnter) {
                    ...
                    setTimeout(() => {
                        if (direction === 0 && this.$refs.search_module.searchData.shipping_number === '') {
                            this.shippingNumberFocus();
                            this.canSetWeight = false;
                        }
                    }, 1500);
