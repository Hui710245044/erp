<template>
    <div class="p-button-dialog">
        <page-dialog v-model="show" width="700px" :title="title.trim()" :min-height="400" :close-on-click-modal="isClose">
            <div class="ml-sm mb-xs" style="text-align: center">
                <span v-for="(item,index) in selectLinks" v-if="item.isShow" :key="index" @click="selectCheck(item)" :style="`${item.active?'border-color:#69f':''}`" class="operate span-link" >{{item.label}}</span>
