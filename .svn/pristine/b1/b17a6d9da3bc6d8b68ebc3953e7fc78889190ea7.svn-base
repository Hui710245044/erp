<template>
    <page class="p-index-ali-publish over-flow-auto"  >
        ...
        <div class="page-fixed">
            ...
            <request-button v-if="showTemp" class="p-wish-btn" :mintime="200" req-key="publish_submit" :disabled="disable" @click.native="save" >保存为草稿</request-button>
            <request-button v-if="isAdd" class="p-wish-btn" :mintime="200" req-key="publish_submit" :disabled="disable" @click.native="publish">提交刊登</request-button>
            <request-button v-if="!isAdd" class="p-wish-btn" :mintime="200" req-key="publish_submit" :disabled="disable" @click.native="update">提交刊登</request-button>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            publish(){
                if(this.flag===1){
                    this.$reqKey('publish_submit',false);
                    return ;
                }
                let flag=true;
                if(flag){
            ...
            get_data(){
                ...
                checkData.vars.forEach((row)=>{
                    ...
                    if(row.listing_info.length>1){
                        ...
                        if(row.sku_attr_info.length>0&&!not_is_null){
                            this.$reqKey('publish_submit',false);
                            flag = false;
                            return this.$message({type:"warning", message: '多SKU产品刊登时，请至少勾选一个（除“发货地”以外的）产品属性！并保证属性值不能重复！'});
                        }
                    }
                ...
                data.vars=clonedata.vars.map((row,row_i)=> {
                        ...
                        info.sku_attributes.forEach(skuAttr => {
                            ...
                            if (findskuattr && findskuattr.is_checked) {
                                ...
                                if (!!skuAttr.changeName) {
                                    if ((skuAttr.changeName&&skuAttr.changeName.search('/') >= 0) || (skuAttr.custom_name&&skuAttr.custom_name.search('/') >= 0)) {
                                        this.$reqKey('publish_submit', false);
                                        flag = false;
                                ...
                                if(skuAttr.customized_pic === 1){
                                    attr_sku.skuImage = (info.d_imgs&&info.d_imgs.length&&info.d_imgs[0].path)||skuAttr.custom_pic;
                                }
                    ...
                    row.aeopNationalQuoteConfiguration && row.aeopNationalQuoteConfiguration.forEach(it => {
            ...
            edit_publish(val,id){
                ...
                this.$http(api_aliexpress_edit_product,parm).then(res=>{
                    ...
                    clones.aeopNationalQuoteConfiguration&&clones.aeopNationalQuoteConfiguration.forEach(it=>{
                    ...
                    Promise.all([template, ser_template, grounp,sales,brands,relation,custom,same_spu]).then(([temp, serv, gro,sale,brand,rela,customer,same_pro]) => {
                        ...
                        this.$refs.goods.init_data(res.goods_id);
                        setTimeout(()=>{
                            this.loading=false;
                        },150)
                    })
