    ...
    <page>
        <search-card @search="search" @enter="search" :params="searchData" :clears="clears">
            ...
            <el-table :data="tableData"
                ...
                <el-table-column label="下载" inline-template width="100">
                    <div>
                        <a :href="`${host}${row.download_url}`" :class="{exportColor:row.status !== 1}" download @click="is_download(row, $event)">下载报表</a>
                    </div>
                </el-table-column>
            </el-table>
...
<script>
    import {api_get_export} from '../../../api/export-manage'
    export default {
        page:{
            devinfo:{
                frontEnd:'张明亮',
