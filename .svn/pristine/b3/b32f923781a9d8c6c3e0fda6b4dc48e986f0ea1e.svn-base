        ...
        <page-dialog title="添加手工订单" v-model="dialog" @open="open" size="large" :close-on-click-modal="false">
            <div>
                ...
                <div class="mt-xs mb-sm bold-font">
                    ...
                    ></label-select>
                    <div class="mt-xs">
                        <label v-if="form.channel_account_id !== ''">平台订单号：</label>
                        <el-input class="inline" v-if="form.channel_account_id !== ''" placeholder="请输入平台订单号" v-model="form.channel_order_number"></el-input>
                    </div>
                </div>
...
<script>
    ...
    import {api_get_number,add_order,api_get_channel,api_get_account,api_get_country } from '../../../api/handwork';
    export default{
        data(){
        ...
        computed:{
            showSite(){
                return this.curChannel === "ebay"||this.curChannel === "ymx"?true:false;
            },
            showAccount(){
                return this.curChannel === "other"?false:true;
            },
            curCurrency(){
                console.log(this.form.currency_code,'this.form.currency_code');
