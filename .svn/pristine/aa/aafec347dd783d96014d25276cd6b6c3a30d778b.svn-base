<template>
    <div class="c-picking-sku-table">
        <div class="mt-xs mb-xs">
            <permission tag="ElButton" @click.native="make_handle" :disabled="markHandle"
        ...
        <el-table class="mb-sm scroll-bar"
            ...
            <el-table-column label="采购员" min-width="50" inline-template>
                <div>{{row.purchase_user_name_list.join()}}</div>
            </el-table-column>
        ...
        <mark-handle v-model="markHandleVisible" :selected-data="selectedData"></mark-handle>
    </div>
</template>
