<template>
    <page class="pabe-pickings-exception">
        ...
        <search-card :params="searchData" @search="init" :clears="clears" class="mb-xs">
            <label-item label="拣货单号：" class="inline">
            ...
            <label-item label="分区：" class="inline ml-sm">
            ...
            <label-item label="差异数：" class="inline ml-sm">
            ...
            <label-item label="拣货人：" class="inline ml-sm">
                <param-account
                        class="width-md"
                        v-sf.shipper_id
            ...
            <label-item label="拣货完成时间：" class="inline ml-sm">
                <el-date-picker
                        class="inline"
                        style="width:140px;"
                        v-sf.date_b
        ...
        <div>
            <permission tag="requestButton"
                        ...
                        :route="apis.url_batch_processing"
                        :disabled="searchData.status === 1 || selectedIds.length === 0"
                        type="primary"
        ...
        <ui-table
            ...
            <template v-for="(data, index) in tableData">
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'']" @click="addClass(index)"
                    ...
                    <!-- 处理者 -->
                    <td>
                        <ui-tips :content="data.handler" :width="65"></ui-tips>
                    </td>
                    <!-- 处理时间 -->
                    <td>
                        <ui-tips :content="data.handler_time" :width="65"></ui-tips>
                    </td>
                    <!-- 最早拣货完成时间 -->
                    <td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            mark_processed() {//标记为处理
                this.$confirm('您将修改已勾选SKU的状态为：已处理，确认此操作吗？', '提示', {
                    ...
                    type: 'warning'
                        ...
                        this.$http(api_batch_processing, data).then(res => {
                            this.$message({
                                type: 'success',
