    ...
    <el-row class="p-purchasing">
        ...
        <ui-table :has-data="dataTable.length>0"
                  ...
                  :heads="[
                        ...
                        {label:'最小起订量',width:6},
                        {label:'采购数量',width:5,isSort:true,type:'desc'},
                        {label:'供应商',width:6},
                        {label:'采购价格',width:7},
                        {label:'采购员',width:4},
                    ...
                    <tr  class="line" :class="item===classData?'active-color':''" @click="addClass(item)">
                        ...
                        <td>
                            <div v-if="searchData.created_plan_status===0">
                                <ui-limit-number v-model="item.min_qty"
                                                 style="width:80px"
                        ...
                        <td>
                            <div v-if="searchData.created_plan_status===0">
                                <purchaser ref="purchaser" v-if="item.select" class="inline" v-model="item.purchaser_id" style="width: 100px"></purchaser>
    ...
    export default{
        ...
        methods: {
            ...
            create_procurement(){/*-------------生成采购计划*/
                ...
                let find = this.dataTable.find(row=>!!row.dataList.find(item=>item.isCheck&&!item.isDiable&&item.purchase_qty<item.min_qty));
                if(!!find){
