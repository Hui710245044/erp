<template>
    <page-dialog size="full" v-model="show" :title="title" @open="open" class="p-form-look" @change="change_dialog"
        ...
        <div v-if="edit">
            <el-button type="primary" :disabled="selectMark === 'stocking'" size="mini" @click.native="add_goods()">
                添加商品
            </el-button>
            <el-button type="primary" :disabled="selectMark === 'stocking'" size="mini"
                       @click.native="showImport=true;">EXCEL导入
            </el-button>
            <el-button type="warning" :disabled="selectMark === 'stocking'" size="mini" @click.native="delete2">
                {{deleteLabel}}
            </el-button>
            <label-item class="fr" label="统一分配采购员：" filterable>
                <purchaser class="inline" :disabled="this.selectMark==='other'" v-model="purchaser_id"
        ...
        <div style="margin-top:5px">
            <el-table :data="filterGoodsLists"
                ...
                <el-table-column type="selection" width="30" v-if="lookData.status!==1"></el-table-column>
                ...
                <el-table-column label="产品SKU/SKU别名" min-width="100" inline-template>
                    <div>
                        <div style="white-space: nowrap">{{row.sku}}</div>
                        <div style="white-space: nowrap">{{row.sku_alias|filterAlias}}</div>
                    </div>
                </el-table-column>
                <el-table-column label="产品名称" min-width="120" inline-template>
                    <div>
                        {{row.sku_name}}
                    </div>
                </el-table-column>
                ...
                <el-table-column label="采购数量" width="50" inline-template>
                ...
                <el-table-column :label="`采购单价${currency?`(${currency})`:''}`" width="70" inline-template>
                    <ui-input v-model="row.purchase_price"
                              :edit="edit"
                              :disabled="this.selectMark==='other'"
                              @input="chang_(row)"
                ...
                <el-table-column label="3月内最低报价" inline-template width="70">
                    <div>
                ...
                <el-table-column label="供应商" min-width="120" inline-template>
                    <div>
                        ...
                        <ui-tip v-else :is-operate="true" :content="row|supFilter" :width="98"
                                @cur-click="modifSupplie(row)"></ui-tip>
                    </div>
                </el-table-column>
                <el-table-column :label="`区间报价${currency?`(${currency})`:''}`" min-width="70" inline-template>
                          <span class="operate"
                                @click="newPrice(row)"
                ...
                <el-table-column label="结算方式" width="80" inline-template>
                    <div>
                ...
                <el-table-column label="操作" width="50" inline-template>
        ...
        methods: {
            ...
            audit_cancel_init(data) {
                ...
                return this.$http(api_post_purchase_plan_cancel, {plans: params}).then(res => {
                    ...
                    params.forEach(row => {
                        let ids = [];
                        this.$emit("edit-update", this.id, ids, row);
                    });
                    return Promise.resolve();
                }).catch(code => {
                    this.$message({
            ...
            save() {
                ...
                return this.$http(api_updata_plan, this.id, saveData).then(res => {
                    if (res.status === 1) {
                        this.$message({
                            type: 'success',
                            message: `操作成功!`
                    ...
                    if (this.lookData.status === 2) {
                        this.$emit("edit-update", this.id, res.id, 2);
                    } else {
                        this.$emit("edit-update", this.id, res.id, 0);
                    }
                    return Promise.resolve();
                }).catch(code => {
                    this.$message({
            ...
            log_init() {
                this.$http(add_look_log, {purchase_plan_id: this.id}).then(res => {
            ...
            change_dialog(val) {
                if (val) {
                    ...
                    if (this.edit && this.dispose) {
                        ...
                        } else if (this.selectMark === 'stocking') {
