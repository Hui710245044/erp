<template>
    <page-dialog size="full"
        ...
        <el-form>
            <el-row>
                <el-col :span="10" v-if="selectMark==='other'">
                ...
                <el-col :span="7">
                    <!--<el-form-item label="采购仓库：" required v-if="selectMark==='stocking'">
                        <span>中转仓</span>
                    </el-form-item>-->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            see_purchase() {
                this.goodsList = [];
                this.$http(api_publish_edit_carrier, {snType: 'purchase_order_code', snText: [this.snText]}).then(res => {
                    if (!this.snText) {
                        this.warehouse_id = '';
                        this.supplier = -1;
                        this.searchShow = false;
            ...
            modifSupplie(data,row) {
                ...
                this.$http(api_getSupplier, skuId).then(res => {
                    ...
                    } else if (!data.supplier && res.message && res.message.length === 1) {
            ...
            save(isApproval) {
                ...
                editSupplierBuyers.forEach(row => {
                    ...
                    amout.tracking_number = row.tracking_number;
            ...
            add_sku(data) {
                this.$http(api_add_sku, data).then(res => {
                    res.forEach(row => {
                        ...
                        row.id = row.sku_id;
                        if (this.selectMark === 'other' && this.searchShow) {
                            let skuId = {
                                sku_id: row.sku_id
                            };
                            this.$http(api_getSupplier, skuId).then(res => {
