<template>
    <page-dialog v-model="show" size="large" :title="titleName" :close-on-click-modal="false" class="sendRules-dialog"  @change="close_send" >
                    ...
                    <el-row>
                        ...
                        <el-col :span="14">
                            <el-form-item label="买家邮箱：">
                                <el-select v-model="emailRules" class="inline" style="width:300px">
                                    <el-option
                ...
                <div>
                    ...
                    <table class="template" style="margin-top:5px">
                        ...
                        <tr  v-for="(data, index) in delayData" :key="index">
                            <td>
                                <i class="el-icon-circle-close icon-close" @click="del_delay(index)" title="删除"></i>
                            </td>
                            <td>
                            ...
                            <td>
                                <el-select v-model="data.template_id" placeholder="请选择回复模板">
                                    <el-option-group
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            emailRulesFun(){
                this.$http(api_send_rules_email).then(res=>{
                    this.email_ = res
                }).catch(code=>{this.$message({type:"error",message:code.message || code})})
            },
            templateFun(){
            ...
            init_config(code){
                if(!this.configs[code]){
                    this.$http(api_send_rules_resources,{code:code}).then(res=>{
            ...
            submit(){
                ...
                if (this.delayData.length > 0) {
                    ...
                    this.delayData.forEach(data=>{
                        if (!data.template_id || (!data.day && !data.hour)) {
                            this.$message({
                              message: '延迟发送规则没填写完善！',
                              type: 'error'
                            });
