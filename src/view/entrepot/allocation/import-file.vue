<template>
    <page-dialog v-model="visible" @open="open" @close="close" :title="importAction.title">
        <div class="file-btn">
            {{`请选择要导入的${importAction.isPDF ? 'PDF' : 'Excel'}文件`}}
            <input type="file" ref="file" :accept="importAction.accept" :multiple="importAction.isMultiple"
                   @change="file_change">
        </div>
        <ul class="files">
                ...
                this.files.forEach(({file}, index) => {
                    ...
                    reader.onload = (e) => {
                        ...
                        if (index === this.files.length - 1) {
                            ...
                            if (this.submit(type, attachment, false) === false) return;
                        }
                    };
                })
            },
