...
<script>
    import {api_report_today} from '../../../api/report-today'
    ...
    export default {
        ...
        methods: {
            async init() {
                try {
                    let res = await this.$http(api_report_today);
                    this.update_time = datef('YYYY-MM-dd HH:mm:ss', Date.now() / 1000);
                    this.warehouseData = res.warehouse_info;
                    console.log(this.warehouseData,'this.warehouseData');
                    this.productivityData = res.capacity_info;
                    this.init_detail();
                } catch (code) {
