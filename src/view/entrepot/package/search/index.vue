<template>
    <page>
        <label-item label="请选择仓库：" class="mb-xs mt-sm">
            <el-select v-model="warehouse_id" filterable class="inline width-sm" @change="select_warehouse">
                <el-option
                        :key="item.value"
        ...
        <search-card @search="go_search" :params="searchData" :clears="clears" class="mb-xs">
            ...
            <label-item label="是否拆包：" class="ml-sm">
            ...
            <label-item label="接收时间：" class="ml-sm mr-sm">
                <el-date-picker
                        v-sf.s_begin_date
        ...
        <el-card v-loading="loading"
            ...
            <el-table
                    ...
                    highlightCurrentRow
                    :default-sort="{prop: 'indexNum', order: 'descending'}">
                ...
                <el-table-column
                        ...
                        inline-template
                        label="是否拆分">
                    <span :class="{redColor:row.is_use}">{{row.is_use | isSplit}}</span>
                </el-table-column>
                <el-table-column
                        label="操作" width="120">
                    <template slot-scope="scope">
                        <permission tag="ElButton"
                                    ...
                                    :class="{packOperate:scope.row.is_use !== 0}">明细
                        </permission>
                        <permission tag="span"
                                    :route="apis.url_parcel_getOrder">|
                        </permission>
                        <permission tag="ElButton"
                                    ...
                                    :class="{packOperate:scope.row.is_use !== 1}">修改
                        </permission>
                        <permission tag="span"
                                    :route="apis.url_parcel_update">|
                        </permission>
                ...
                inputTimeStart: {
                    disabledDate: (time) => {
                        if (this.searchData.s_end_date) {
                            return time.getTime() > this.searchData.s_end_date;
                        } else {
                            return false
                        }
                    }
                ...
                inputTimeEnd: {
                    disabledDate: (time) => {
                        if (this.searchData.s_begin_date) {
                            return time.getTime() < this.searchData.s_begin_date;
                        } else {
                            return false
                        }
                    }
        ...
        methods: {
            ...
            init() {
                ...
                this.$http(api_parcel_get, data).then(res => {
                    ...
                    this.tableData = res.list.reverse();
                    let i = 1;
