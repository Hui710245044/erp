    ...
    <div>
        <search-card :params="searchData" :clears="clears" @search="search" @enter="search">
            <div class="mt-xs mb-xs">
                <label-buttons label="平台：" @select="change_status" :buttons="channels" :max='14'></label-buttons>
            </div>
        ...
        methods: {
            ...
            init_status(){
                this.$http(api_get_channel).then(res=>{
                    this.channels = res.filter(item=> item.value !== 5&&item.value !== 6&&item.value !== 9&&item.value !== 10&&item.value !== 11&&item.value !== 12&&item.value !== 13&&item.value !== 14);
                }).catch(code => {
