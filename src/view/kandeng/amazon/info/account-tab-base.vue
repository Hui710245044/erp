    ...
    <div>
        ...
        <div class="p-head-border-color">
            <el-form :label-width="labelWidth" :model="form" :rules="rules_1" ref="base_info_1">
                <el-form-item label="引用历史刊登：" v-if="curPage==='not'">
                    <el-button type="primary"
                               size="mini"
                               @click.native="quote_old">一键引用（{{referableNum}}）</el-button>
                </el-form-item>
                <el-form-item label="范本引用：" v-if="!showSite&&!!form.doc_id&&curPage==='not'">
                    <el-button type="primary"
                               size="mini"
                               @click.native="quote_model">一键引用范本</el-button>
                </el-form-item>
                <el-form-item label="平台账号：" v-if="!showSite">
                ...
                <el-form-item label="分类模板：" required prop="category_template_id">
            ...
            <el-form :model="form" :label-width="labelWidth">
                ...
                <template v-for="(item,index) in form.basic">
                        <el-form-item v-if="is_show(item)"
                                      ...
                                      :rules="{required: item.require === 1, message:'必填选项', trigger:'blur'}"
                                      :label="`${item.name}：`">
    ...
    export default{
            ...
            return {
                rules_1: {
                    ...
                    category_template_id: [
                        {required: true, message: '分类模板为必填项', trigger: 'change', type: 'number'}
                    ]
        ...
        methods: {
            ...
            initial_uppercase(index){
                if(!this.form.descript[index].value) return this.$message({message:"请输入刊登标题", type:'warning'});
            ...
            * */
            get_attr_product(){
                ...
                this.get_product_attribute(productType,this.form.siteId).then(id=>{
                    /*初始化产品默认模板属性
                    * */
                    if(!id)return this.$message({type:"warning",message:"请先设置一个默认的产品模板"});
