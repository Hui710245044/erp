...
//                    is_plan_publish:"",
        ...
        methods: {
            ...
            init_data(goods_id){//在info/index页面使用
                this.especialIds = [goods_id];
                this.get_account().then(res=>{
            ...
            get_account(){//在 info/index页面也有请求
                return this.$http(api_express_get_accounts,{spu:this.relevanceForm.spu||this.relevanceForm.goods_spu}).then(res => {
            ...
            //计算销售价
            get_pricing_rules(account,skuList){
                let params = {
                    channel_id:4,
                    account_id:account,
                    site_code:0,
                ...
                this.$http(api_pricing_rules,params).then(res => {
                    this.allPricingRules[account] = res;
                }).catch(code => {
                    console.log(code);
                })
            },
